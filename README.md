# 👋 Hi, I'm Hafiz Salih

A highly motivated Software Engineering student specializing in web and mobile app development, passionate about creating innovative, user-centric solutions.

---

## About Me

🎓 Software Engineering student at Metropolia University of Applied Sciences, Helsinki, Finland (Graduation expected Dec 2025)  
🌍 Currently interning at HEIG-VD, Switzerland (SMAPSHOT - Scientific Mediation Experience)  
💡 Passionate about intuitive UI/UX design, interactive web & mobile experiences, and Agile collaboration  
🛠️ Proficient in the MERN stack (MongoDB, Express.js, React.js, Node.js), Vue.js, OpenLayers, and a variety of modern web and mobile development technologies

---
## 🔥 Current Work &nbsp; ![In Progress](https://img.shields.io/badge/status-in%20progress-yellow)

### Augmented Reality Historical Images (2025)

A web-based AR platform that allows users to view historical photos overlaid on their camera view when visiting the locations where the original photos were taken.

- 🧩 Built with **React.js**, integrating the **SMAPSHOT API** to fetch historical photos
- 📷 Implements camera access and location tracking
- 🕶️ AR overlays using **WebAR/WebXR technologies**
- 🗺️ Mapping via **OpenLayers**

**Key Skills:** React.js, Git, HTML/CSS, Responsive Web Design, REST APIs, OpenLayers, WebXR API, Geolocation API, DeviceOrientation API, MediaDevices API, A-Frame, User Interface Design
## 🛠️ Skills

**Frontend Development**  
![HTML5](https://img.shields.io/badge/-HTML5-E34F26?logo=html5&logoColor=fff)
![CSS3](https://img.shields.io/badge/-CSS3-1572B6?logo=css3&logoColor=fff)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?logo=javascript&logoColor=000)
![React](https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=000)
![Vue.js](https://img.shields.io/badge/-Vue.js-4FC08D?logo=vue.js&logoColor=fff)
![Bootstrap](https://img.shields.io/badge/-Bootstrap-7952B3?logo=bootstrap&logoColor=fff)
![TailwindCSS](https://img.shields.io/badge/-Tailwind-38B2AC?logo=tailwind-css&logoColor=fff)
![OpenLayers](https://img.shields.io/badge/-OpenLayers-1F6FEB?logo=OpenStreetMap&logoColor=fff)
![A-Frame](https://img.shields.io/badge/-A--Frame-E87114?logo=aframe&logoColor=fff)
![Responsive Web Design](https://img.shields.io/badge/-Responsive%20Web%20Design-2396F3?logo=responsive&logoColor=fff)
![User Interface Design](https://img.shields.io/badge/-UI%20Design-FF4088?logo=figma&logoColor=fff)

**Web APIs & AR Technologies**  
![REST API](https://img.shields.io/badge/-REST%20API-005571?logo=api&logoColor=fff)
![WebXR](https://img.shields.io/badge/-WebXR-FF6F00?logo=webxr&logoColor=fff)
![Geolocation API](https://img.shields.io/badge/-Geolocation%20API-43A047?logo=googlemaps&logoColor=fff)
![DeviceOrientation API](https://img.shields.io/badge/-DeviceOrientation%20API-FFD600?logo=gyroscope&logoColor=000)
![MediaDevices API](https://img.shields.io/badge/-MediaDevices%20API-00BCD4?logo=camera&logoColor=fff)

**Backend & DevOps**  
![Node.js](https://img.shields.io/badge/-Node.js-339933?logo=node.js&logoColor=fff)
![Express](https://img.shields.io/badge/-Express-000?logo=express&logoColor=fff)
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?logo=mongodb&logoColor=fff)
![SQL](https://img.shields.io/badge/-SQL-003B57?logo=mysql&logoColor=fff)
![Docker](https://img.shields.io/badge/-Docker-2496ED?logo=docker&logoColor=fff)
![Jenkins](https://img.shields.io/badge/-Jenkins-D24939?logo=jenkins&logoColor=fff)
![GitHub](https://img.shields.io/badge/-GitHub-181717?logo=github&logoColor=fff)
![GitLab](https://img.shields.io/badge/-GitLab-FC6D26?logo=gitlab&logoColor=fff)
![Git](https://img.shields.io/badge/-Git-F05032?logo=git&logoColor=fff)
![CI/CD](https://img.shields.io/badge/-CI%2FCD-2088FF?logo=github-actions&logoColor=fff)
![JUnit5](https://img.shields.io/badge/-JUnit5-25A162?logo=java&logoColor=fff)

**Mobile Development**  
![Android](https://img.shields.io/badge/-Android-3DDC84?logo=android&logoColor=fff)
![Kotlin](https://img.shields.io/badge/-Kotlin-7F52FF?logo=kotlin&logoColor=fff)
![iOS](https://img.shields.io/badge/-iOS-000000?logo=apple&logoColor=fff)
![Swift](https://img.shields.io/badge/-Swift-FA7343?logo=swift&logoColor=fff)

**Data & Visualization**  
![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=fff)
![Pandas](https://img.shields.io/badge/-Pandas-150458?logo=pandas&logoColor=fff)
![Power BI](https://img.shields.io/badge/-Power%20BI-F2C811?logo=powerbi&logoColor=000)
![Excel](https://img.shields.io/badge/-Excel-217346?logo=microsoft-excel&logoColor=fff)

---

**UI/UX & Design**  
![Figma](https://img.shields.io/badge/-Figma-F24E1E?logo=figma&logoColor=fff)
![Canvas](https://img.shields.io/badge/-Canvas-FFA500?logo=canvas&logoColor=fff)
![Miro](https://img.shields.io/badge/-Miro-050038?logo=miro&logoColor=fff)

---

**Collaboration & Project Management Tools**  
![Jira](https://img.shields.io/badge/-Jira-0052CC?logo=jira&logoColor=fff)
![Trello](https://img.shields.io/badge/-Trello-0052CC?logo=trello&logoColor=fff)
![Notion](https://img.shields.io/badge/-Notion-000000?logo=notion&logoColor=fff)
![Slack](https://img.shields.io/badge/-Slack-4A154B?logo=slack&logoColor=fff)
---

## 🏆 Featured Projects

- **SMAPSHOT Project (2025):**  
  Internship at HEIG-VD, Switzerland  
  _Vue.js, UI Design, Gamification, Agile_
  - Developed interactive tutorials for scientific mediation and georeferencing
  - Implemented gamified elements to enhance user engagement
  - Applied UI design principles to build intuitive interfaces
  - Collaborated in an international Agile team  
  [View Code](https://gitlab.com/mediacomem/smapshot/smap_checker) &nbsp;&bull;&nbsp; [Live Demo](https://smapshot-beta.heig-vd.ch/geo_game#/)

- **WeatherMate App (2024):**  
  Android app for real-time weather & 7-day forecast  
  _Kotlin, Jetpack Compose, Dagger Hilt_  
  [View Code](https://github.com/HafizDa/WeatherMate)

- **TaskFlow App (2024):**  
  iOS task manager with reminders, multilingual support  
  _Swift, SwiftUI, SwiftData_  
  [View Code](https://github.com/Mamita123/TaskFlow)

- **Fitness Activity App (2023):**  
  MERN stack fitness tracker with Agile/DevOps  
  [View Code](https://github.com/HafizDa/Fitness_First_Merged)

- **Supermarket Simulation (2023):**  
  Java/JavaFX queuing simulation  
  [View Code](https://github.com/HafizDa/SupermarketSimulator)

- **Barbershop Website (2023):**  
  MERN stack booking/auth (JWT/OAuth)  
  [View Code](https://github.com/HafizDa/barbershop)

- **Travel Agency Website (2021):**  
  Responsive travel agency site (HTML, CSS, JS)  
  [View Code](https://github.com/HafizDa/Vacationer-travel-agency)

---

## 💼 Experience

**Intern - Web Developer**  
HEIG-VD, Switzerland _(Jan 2024 – Present)_  
• Developing interactive tutorials (Vue.js, React, UI design, gamification)  
• Agile collaboration in an international team



---

## 🎓 Education

**BSc in Information and Communication Technology (2021–2025)**  
Metropolia University of Applied Sciences, Helsinki  
_Focus: Software Engineering, web & mobile development_

**ICT Specialist (2020–2021)**  
Taitotalo, Helsinki

---

## 🌐 Languages

- **Tigrinya:** Native  
- **English:** Professional  
- **Finnish:** Professional

---

## 📫 Contact

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?logo=linkedin&logoColor=fff)](https://www.linkedin.com/in/hafizsalih)
[![Portfolio](https://img.shields.io/badge/-Portfolio-24292e?logo=githubpages&logoColor=fff)](https://hafida.github.io/Mp-portfolio)
[![Gmail](https://img.shields.io/badge/-salehhafiz5@gmail.com-D14836?logo=gmail&logoColor=fff)](mailto:salehhafiz5@gmail.com)

---

<!--
**HafizDa/HafizDa** is a ✨ special ✨ repository because its README.md (this file) appears on your GitHub profile.
-->
